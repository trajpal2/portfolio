    ...
    <header className={`header${scrolled ? " is-scrolled" : ""}`}>
      <nav className="nav" aria-label="Primary">
        ...
        <ul className={`nav-menu${menuOpen ? " is-open" : ""}`} id="nav-menu">
          {NAV_LINKS.map((link) => {
            const active = link.end ? pathname === link.to : pathname === link.to || pathname.startsWith(`${link.to}/`);
            return (
              <li key={link.to}>
                <Link href={link.to} className={active ? "is-active" : undefined} onClick={closeMenu}>
                  {link.label}
                </Link>
              </li>
            );
